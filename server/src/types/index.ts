...
export interface TokenMetadata {
    ...
    media_hash: number | string; // Base64-encoded sha256 hash of content referenced by the `media` field. Required if `media` is included.
    copies: number | string; // number of copies of this set of metadata in existence when token was minted.
    ...
    reference_hash: number | string; // Base64-encoded sha256 hash of JSON from reference field. Required if `reference` is included.
}
